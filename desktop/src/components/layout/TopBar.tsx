    ...
    >
      ...
      <div className="flex items-center gap-3" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
        <button
          ...
          className="flex items-center gap-1.5 text-[11px] font-medium bg-slate-50 border border-slate-200 px-2.5 py-1 rounded-full text-slate-600 hover:bg-slate-100 transition-colors"
        >
          <StatusIndicator status={connected ? 'connected' : 'disconnected'} size="sm" />
          <span>{connected ? `端口 ${port}` : '未连接'}</span>
        </button>
